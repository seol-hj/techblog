...
1. MongoDB도 **인덱스 없으면 느리다**
2. JOIN은 쿼리가 아니라 **설계 문제**
3. 외부 오픈 금지 (`bindIp`)
4. 인증 없는 MongoDB = 사고 예약
...
> 운영 팁: 실습이 끝나면 bindIp: 0.0.0.0는 반드시 제한하세요.
...
![](20_Experience/MGC_SA_Bootcamp/3_데이터베이스/img/20260212-13.png)

![](20_Experience/MGC_SA_Bootcamp/3_데이터베이스/img/20260212-14.png)
...
## 2-12. 운영 관점 핵심 체크리스트 (강의 포인트)

1. **Replica Set = 고가용성(HA) + 데이터 복제**
2. Failover의 핵심 조건은 **과반수(majority)**
3. Replica가 있어도 **백업은 별개**(삭제/오염도 복제됨)
4. 실습에서는 열어둔 `bindIp: 0.0.0.0`를 운영에선 반드시 제한
5. 인증/권한(authorization) 적용은 다음 장(보안)에서 진행 권장

---

## 2-13. 실습 과제(제출형)

1. 현재 PRIMARY 노드를 찾아 스크린샷(또는 출력 로그) 제출
2. PRIMARY를 중지하고, 새 PRIMARY가 누군지 확인
3. Failover 후 insert가 되는지 증명(데이터 출력)
4. “2대로는 왜 Failover가 어려운가(과반수 관점)”를 서술하라
